 ...
 */
int TESTRIG_API decode_error(const uint32_t error_code, char* error_msg, uint8_t max_size);

/*
 *	@brief Decode and print an error code's corresponding message.
 */
int TESTRIG_API print_error(const uint32_t error_code);

/*
 *	@brief Reset a device.
 */
uint32_t TESTRIG_API reset_device(void* device_handle, struct controller* controller_in);

/*
 *	@brief Setup that's tooled to our configuration.
 */
uint32_t TESTRIG_API setup_testrig_as_can_gateway(
	struct controller controllers[3], void* handles[3], int default_init);

/*
 *	@brief Cleanup that's tooled to our configuration.
 */
uint32_t TESTRIG_API cleanup_testrig(
